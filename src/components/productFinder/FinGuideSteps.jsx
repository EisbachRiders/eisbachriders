...
function getSteps(boardType, waveSize, surfLocation) {
  return [
    boardType === '' ? 'Select your board type' : boardType,
    waveSize === '' ? 'What wave height are you surfing?' : waveSize,
...
export default function FinGuideSteps({ onClose }) {
  const classes = useStyles()
  const [activeStep, setActiveStep] = React.useState(0)
  const [boardType, setBoardType] = React.useState('')
  const [waveSize, setWaveSize] = React.useState('')
  const [surfLocation, setSurfLocation] = React.useState('')
  const steps = getSteps(boardType, waveSize, surfLocation)
  ...
  if (isSurfboard) {
    ...
    } else if (waveSize === '6-7ft') {
      result = '6ft Surfboard Leash'
    } else if (waveSize === '7-8ft') {
      result = '7ft Surfboard Leash'
    } else if (waveSize === '>8ft') {
